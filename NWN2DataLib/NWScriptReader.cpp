...
#define BYTESWAP_ULONG( v )   _byteswap_ulong( v )
#define BYTESWAP_USHORT( v )  _byteswap_ushort( v )
  ...
  m_Analyzed( false )
{
	...
	try
	{
		...
		if (FileSize != BYTESWAP_ULONG( Header.FileSize ))
		{
			StringCbPrintfA(
				ErrorMsg,
				sizeof( ErrorMsg ),
