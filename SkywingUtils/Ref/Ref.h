...
namespace swutil
{
...
#else
		return InterlockedIncrement( Addened );
#endif
	}
	...
	private:
		...
		InterlockedAddReference(
			__inout ULONG_PTR volatile * Reference
			)
		{
...
//	private: // For internal use only, do not use.
	...
	//
	// The purpose of these C_ASSERTs is to ensure that the layout of the
	// SharedBuffer does not change.  A layout change requires that DLL
	// interfaces exposing SharedPtr be recompiled (and become thus
	// incompatible).
	//
...
//	private: // Do not access these members publicly.
	...
	//
	// The purpose of these C_ASSERTs is to ensure that the layout of the
	// SharedPtr does not change.  A layout change requires that DLL
	// interfaces exposing SharedPtr be recompiled (and become thus
	// incompatible).
	//
